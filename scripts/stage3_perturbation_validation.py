...
def main():
    ...
    for op_name, _ in TIER_B_OPS:
        op_stats[op_name] = {
            "applicable": 0,
            "correct_ordering": 0,  # perturbed recall < 1.0 (legacy rule)
            "detected_by_f1": 0,    # v3 rule: recall < 1.0 OR a contrastive entailed
    ...
    for i, (nl, raw_fol, story_preds) in enumerate(premises):
        ...
        # v3: enable contrastives so the strictly_stronger probes can detect
        # logically stronger perturbations (e.g. B_restrictor_drop) via the
        # precision channel.
        result = generate_test_suite_from_gold(
            raw_fol, nl=nl, verify_round_trip=False, with_contrastives=True, timeout_s=5
        )
        ...
        for op_name, op_func in TIER_B_OPS:
            ...
            perturbed_f1 = report.f1 if report.f1 is not None else perturbed_recall
            op_stats[op_name]["avg_perturbed_recall"].append(perturbed_recall)
            op_stats[op_name]["avg_perturbed_f1"].append(perturbed_f1)

            # v3 rule: detected if recall < 1.0 OR f1 < 1.0 (i.e. some
            # contrastive was entailed by the perturbed candidate).
            if perturbed_f1 < 1.0:
                op_stats[op_name]["detected_by_f1"] += 1
    ...
    print("=" * 70)
    print()
    print(f"{'Operator':<22} {'Applic':>7} {'recRate':>8} {'f1Rate':>8} {'avgRec':>7} {'avgF1':>7}")
    print("-" * 65)

    for op_name, stats in op_stats.items():
        applicable = stats["applicable"]
        if applicable == 0:
            print(f"{op_name:<22} {'0':>7} {'-':>8} {'-':>8} {'-':>7} {'-':>7}")
            continue
        correct = stats["correct_ordering"]
        f1_det = stats["detected_by_f1"]
        rate_recall = correct / applicable
        rate_f1 = f1_det / applicable
        avg_recall = (
            sum(stats["avg_perturbed_recall"]) / len(stats["avg_perturbed_recall"])
            if stats["avg_perturbed_recall"] else 0
        )
        avg_f1 = (
            sum(stats["avg_perturbed_f1"]) / len(stats["avg_perturbed_f1"])
            if stats["avg_perturbed_f1"] else 0
        )
        print(f"{op_name:<22} {applicable:>7} {rate_recall:>7.1%} {rate_f1:>7.1%}"
              f" {avg_recall:>7.3f} {avg_f1:>7.3f}")
    ...
    report_data = {
        ...
        "per_operator": {
            name: {
                ...
                "correct_ordering_recall": s["correct_ordering"],
                "detected_by_f1": s["detected_by_f1"],
                "same_score": s["same_score"],
                "higher_score": s["higher_score"],
                "detection_rate_recall": s["correct_ordering"] / s["applicable"] if s["applicable"] else 0,
                "detection_rate_f1": s["detected_by_f1"] / s["applicable"] if s["applicable"] else 0,
                "avg_perturbed_recall": (
                    sum(s["avg_perturbed_recall"]) / len(s["avg_perturbed_recall"])
                    if s["avg_perturbed_recall"] else None
                ),
                "avg_perturbed_f1": (
                    sum(s["avg_perturbed_f1"]) / len(s["avg_perturbed_f1"])
                    if s["avg_perturbed_f1"] else None
                ),
            }
