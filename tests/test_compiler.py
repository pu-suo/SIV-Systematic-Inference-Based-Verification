...
def test_fix_A_universal_binary_binding_emits_universal_shape():
    ...
    assert any(
        f.startswith("all x.") and "Students(x) ->" in f and "Books(y)" in f and "Read(x, y)" in f
        for f in pos
...
def test_fix_G1_conditional_with_binary_facts_emits_macro_test():
    ...
    assert fol.startswith("all x."), f"Expected universally quantified test; got {fol}"
...
def test_fix_G2_type_A_picks_universal_entity_not_first():
    ...
    assert any(t.fol_string.startswith("all x.(Students(x) -> ") for t in macro_tests), (
